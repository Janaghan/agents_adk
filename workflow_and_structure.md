...
│   ├── hotel_agent.py       # Hotel accommodation specialist (tool: search)
│   ├── planner_agent.py     # Day-by-day planner (sub-agent: itinerary)
│   ├── itinerary_agent.py   # Activity planner (tool: search)
│   ├── budget_agent.py      # Budget verifier (tools: math, currency, exit_loop)
...
│   └── get_current_time.py  # Mock city time provider
│
...
graph TD
    ...
    subgraph Phase 1: Research
        ...
        Research --> GoogleSearch1[GoogleSearchAgentTool]

        Weather --> WeatherTool[weather_tool.py <br/><i>get_weather</i>]
        Hotel --> GoogleSearchHotel[GoogleSearchAgentTool]
    end

    subgraph Phase 2: Budget Loop
        Coord --> BudgetLoop[budget_loop <br/><i>LoopAgent (max 3 iterations)</i>]
        ...
        Itinerary --> GoogleSearchItinerary[GoogleSearchAgentTool]
        
        Budget --> CalcMath[calculate_math.py]
        Budget --> CurrTool[currency_tool.py]
...
| :--- | :--- | :--- | :--- |
| **intake_agent** | `Agent` | `gemini-2.5-flash` | **First step.** Collects destination, travel dates, and budget from the user. Asks follow-up questions for any missing details and confirms a Trip Brief before handing off. |
| **coordinator_agent** | `SequentialAgent` | - | Master orchestrator. Runs `intake_agent` → `research_agent` → `budget_loop` → `summary_agent` sequentially. |
| **research_agent** | `Agent` | `gemini-2.5-flash` | Coordinates weather & hotel lookups. Gathers research data and compiles a report. |
| **weather_agent** | `Agent` | `gemini-2.5-flash` | Sub-agent to `research_agent`. Uses `get_weather` tool and suggests packing advice before handoff. |
| **hotel_agent** | `Agent` | `gemini-2.5-flash` | Sub-agent to `research_agent`. Uses Google Search tool to find actual hotel rates and availability. |
| **budget_loop** | `LoopAgent` | - | Handles iterative feedback loop containing `planner_agent` and `budget_agent` to enforce budget limits. |
| **planner_agent** | `Agent` | `gemini-2.5-flash` | Uses `itinerary_agent` to build daily plans, ensuring costs are listed. Updates plan if budget is exceeded. |
| **itinerary_agent** | `Agent` | `gemini-2.5-flash` | Sub-agent to `planner_agent`. Uses Google Search tool to find activities, schedules, and local options. |
| **budget_agent** | `Agent` | `gemini-2.5-flash` | Sums costs, checks against user budget limit, and calls `exit_loop` if valid, or requests refinement. |
| **summary_agent** | `Agent` | `gemini-2.5-flash` | Gathers research and planned itineraries to format a cohesive and premium markdown output. |
...
3. **Intake Phase**:
   ...
   - Once collected, it transfers control to `coordinator_agent`.
4. **Research Phase**:
   - `research_agent` kicks off.
   - It calls `weather_agent` (which calls `get_weather`) to find Goa's weather.
   - It calls `hotel_agent` (which queries Google Search) to list top-rated hotels in Goa.
   - It aggregates findings and returns to the coordinator.
4. **Iterative Planning & Budget Validation Loop**:
...
- **Clean Structure**: Separation between `agents`, `tools`, and `workflows` matches the recommended ADK workflow setup perfectly.
...
- **Effective Loop constraints**: The `LoopAgent` prevents infinite loops by capping iterations at 3, and utilizes `exit_loop` properly inside the budget checker.
